...
$stmt = $db->stmt_init();
$stmt->prepare('SELECT `version`, `rom`, `url`, `md5`, `changelog` FROM `roms` WHERE `romid` = ? AND `device` = ? ORDER BY `version` ASC LIMIT 1');
$stmt->bind_param('ss', $romid, $device);
$stmt->execute();
$stmt->bind_result($version, $rom, $url, $md5, $changelog);
